...
class CmdColor(BaseCommand):
    ...
    Determine the color of a target
    """
    ...
    def func(self):
        ...
        if not self.target or self.target == "here":
            string = f"Your color is {caller.db.color}"
        else:
            target = caller.search(self.target)
            if not target: 
                return 
            if(target.db.color == None):
                string = f"{target.key}\'s color doesn't matter"
            else:
                string = f"{target.key}\'s color is {target.db.color}"
...
class CmdTouch(BaseCommand):
    ...
    def func(self):
        ...
        if not self.target or self.target == "here":
            string = f"You pat yourself on the back"
        else:
            target = caller.search(self.target)
            if not target: 
                return 
